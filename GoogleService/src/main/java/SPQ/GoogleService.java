...
public class GoogleService extends Thread {
	...
	public void run() {
		try {
			...
			System.out.println("   - GoogleService - Received data from '" + tcpSocket.getInetAddress().getHostAddress() + ":" + tcpSocket.getPort() + "' -> '" + userDTO.toString() + "'");					
			...
			System.err.println("   # GoogleService - ClassNotFound error:" + e.getMessage());
		} finally {
			try {
				tcpSocket.close();
			} catch (IOException e) {
